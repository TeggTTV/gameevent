import { endGameByFacilitator } from '@/lib/gameState';
...
export async function POST(
  ...
  try {
    ...
    const result = endGameByFacilitator(code, claims.facilitatorId);
    if ('error' in result) {
      return Response.json({ error: result.error }, { status: 400 });
    }

    return Response.json({ status: result.room.status });
  } catch {
